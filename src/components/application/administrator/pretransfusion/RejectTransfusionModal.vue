<template>
  <v-dialog v-model="visible" persistent max-width="360">
    <v-card data-testid="card">
      ...
      <v-card-title data-testid="title">Reportar não-conformidade</v-card-title>

      <v-card-subtitle class="text-body-1 pt-4" data-testid="subtitle">
        Você realmente deseja reportar não conformidade para está requisição no
        sistema? Justifique:
      </v-card-subtitle>
...
export default {
  ...
  created() {
    this.unsubscribe = this.$store.subscribeAction({
      after: (action) => {
        if (action.type === 'modal/rejectTransfusionRequisition') {
          this.visible = true;
        }
      },
    });
    ...
    async submit() {
      try {
        if (this.$refs.form.validate()) {
          this.loading = true;
          await this.rejectTransfusionRequisition({
